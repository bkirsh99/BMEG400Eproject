...
For these purposes, we reduced the scope to a subset of samples, namely the African American (ASW), African (YRI), and European American (CEU) populations. The main goal of this project is to examine the relationship between obesity-related variants and ancestry and elucidate the genetic underpinnings of obesity susceptibility across populations.

Ultimately, this re-analysis will illustrate the spectrum of genetic diversity between geographically distant genomes to reveal ancestral biological signatures and systematic differences underlying the obesity epidemic. In doing so, we hope to encourage diversity and inclusion in genomics research, as well as urge for the investigation of genetic variants with extreme allele frequency differences (EAFD) between populations to help address health disparities. This analysis will also explore the correlation between obesity prevalence and effect allele frequency across the three populations.

# Methods

This project implements a pipeline that uses the R programming language external R packages to automate key components of data processing, analysis, and visualization. Furthermore, it utilizes a wide set of user-defined helper functions that are documented and showcased in the Appendix.

It consists of a re-analysis of the original study, where only 3 out of the 26 populations are investigated to explore the impact of ancestral genetics rather than environmental factors on obesity prevalence and a brief analysis into correlation of population obesity prevalence and effect allele frequency using data collected from the [Global Health Observatory, World Health Organization](https://apps.who.int/gho/data/view.main.CTRY2450A?lang=en)
...
By utilizing pre-compiled, but not pre-processed data, we overcame computationally-intensive and resource-demanding challenges associated with retrieving data from the 1000 Genomes Project. At the same time, we ensured that this re-analysis was conducted on the raw data set used by the original publication prior to any major filtering or manipulation steps. Obesity prevalence data from WHO survey was also collected from [Global Health Observatory, World Health Organization](https://apps.who.int/gho/data/view.main.CTRY2450A?lang=en).

As a limitation, data for the calculation of composite risk scores was not published in the original study. The formula used by the authors required information about the 225 SNPs for each person present in the 1000 Genomes Project, including the number of copies of effect alleles at obesity-associated variants and their effect size. Since these were not retrievable for the aforementioned reasons, two additional analyses on the enrichment and depletion data were performed. An investigation of Venn diagrams and allele frequency distribution plots were also added to supplement the re-analysis.
...
In this re-analysis, many hypothesis can be made based on the graphical depiction of data. Thus, we created different plots to assist in drawing conclusions about ancestry and admixture as they relate to obesity prevalence. The helper functions are available in [Appendix C: makeVenn, makeHeatmap, plotDist, plotCorrPops](#appendix-c-makevenn-makeheatmap-plotdist).
...
For correlation, the most recent survery data available, (2016) to be more representative of current obesity prevalence (%). Combined data from both sexes were used with the obesity prevalence (%) values from the super populations, (AFR and EUR) of the populations in study to provide a broader perspective on obesity prevalence among populations.
...
# argument(s): "df": a data frame with the BMI ≥ 30, age-standardized
...
plotCorrPops <- function(df,freq,sex,year,superPop){
  ...
  meanAF <- colMeans(mutate_all(freq, function(x) as.numeric(as.character(x))))
  meltAF <- reshape2::melt(as.matrix(meanAF), value.name = "MeanEffectAlleleFrequency")
  AFR <- subset(filtdf, REGION==superPop[2])
  EUR <- subset(filtdf, REGION==superPop[1])
  AFR <- na.omit(AFR)
  EUR <- na.omit(EUR)
  AFR_mean <- mean(AFR$Numeric)
  EUR_mean <- mean(EUR$Numeric)
  
  avg_prev <- c(AFR_mean,EUR_mean,AFR_mean)
  corr_data <- cbind(meltAF,avg_prev)
  
  plot <- ggplot(corr_data, aes(x = MeanEffectAlleleFrequency, y = avg_prev)) + geom_point() + geom_text(aes(label=Var1),hjust=0.5, vjust=2) + geom_smooth(method=lm, se=FALSE) + ggtitle("") +
xlab("Average Super Population Effect Allele Frequency") + ylab("Average Super Population Obesity Prevalence %") + ggtitle("Plot of effect allele frequency distribution")
  return(plot)
}
...
The average obesity prevalence of obesity increases from African sub populations such as ASW and YRI with lower significant effect allele frequency to those from European sub populations such as CEU. This suggests that the influence of an individual's ancestry and genetics on their effect allele frequency, may play a role in obesity prevalence among populations.

```{r, eval=TRUE, tidy = TRUE, cache=FALSE, fig.cap="Correlation plot of WHO obesity prevelence and Effect Allele Frequency for the three populations", message=F, warning=F}
sex <- c("BTSX")
year <- c("2016")
superPop <- c("EUR", "AFR")
corrPlotPops <- plotCorrPops(WHO_BMI, freqInfo,sex ,year ,superPop)
corrPlotPops 
```
...
The heat map showing significant enrichment and depletion of the effect alleles of 225obesity risk SNPs across the 26 populations in generated from this study does not completely match the comparative heatmap generated by the original study. There may be many factors that play in this disparity. Firstly, it may be due in fact to the method used to test the hypergeometric model. The authors of the original study stated that a hypergeometric model was used however, this can be done through a Fisher's Exact test or through hypergeometric testing, which in theory should provide the same result. The method chosen for this analysis was hypergeometric testing and the errors may have occurred in preparing our phyper function. We used  data from the GWAS catalog which is a curated database. The information about effect allele frequency can be changed by the discretion of curators of the database, to ensure the data is clean and up to date, from the time of the original authors' study and our analysis. Any changes in data would lead to different results provided by phyper because of how sensitive it operates on data. Another potential reason why our generated heatmap may not be identical to the original may be due to the method of generation. The original authors used dChip which uses a model base approach to which allows for pooling across multiple arrays, [dCHIp](https://doi.org/10.1073/pnas.011404098). We used heatmap.3's library to create our heat map. The model based approach includes passing the data through different steps and making quality checks along the way which can affect being fed into the clustering algorthm, whereas our data is depedent on the implementation of the heatmap 3 algorthm implementation which is actively being updated. Another potential discrepancy may be the distance function in clustering. Our generated heat map uses the default dist function, which uses euclidean distancing, which can be seen in difference between the lines of the dendrogram created by our heat map and the original heat map. The authors may have chosen another distancing function to apply to the clustering that may not have been specified.
```{r, eval = TRUE,echo=FALSE}
...
Although, the correlation between the three populations provides some insight, it is still limited. The original study also investigated correlation between WHO-surveyed obesity prevalence and population-level average of composite genetic risk scores. This comparison highlights how genetic risk affects obesity prevalence across wider set of populations which would give a deeper understanding of genetic effects onobesity prevalence.

Thus, ancestral genetics might in fact play a role in the disease susceptibility. However, obesity and obesity-related traits are the result of combined effects of multiple different genetic variants and their interaction with surrounding environmental variables, so socio-economic factors such as diet, climate, local pathogens and lifestyle are large contributors as well.
...
plotCorrPops <- function(df,freq,sex,year,superPop){
  ...
  plot <- ggplot(corr_data, aes(x = MeanEffectAlleleFrequency, y = avg_prev)) + geom_point() + geom_text(aes(label=Var1),hjust=0.5, vjust=2) + geom_smooth(method=lm, se=FALSE) + ggtitle("") +
xlab("Average Super Population Effect Allele Frequency") + ylab("Average Super Population Obesity Prevalence %")
  return(plot)
}
```
