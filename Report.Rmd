...
makeHeatmap <- function(df,groupColours){
  ...
  if(missing(groupColours)){
    ...
    heatmap3(y, Rowv = as.dendrogram(hr), Colv = as.dendrogram(hc), ColSideColors=groupColours,col=col_fun,scale="row",margins=c(3,7),cexRow=0.8,cexCol=1,file="heatmap.pdf")
  }
    dev.off()
}
...
sigRes
annotation <- as.data.frame(fread("pop.txt",select=c(1,3)))
