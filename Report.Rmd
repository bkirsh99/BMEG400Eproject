...
makeHeatmap <- function(df,groupColours){
  heatmapDf <- df
  ...
  if(missing(groupColours)){
    ...
    heatmap3(y, Rowv = as.dendrogram(hr), Colv = as.dendrogram(hc), ColSideColors=groupColours,legendfun=function()showLegend(legend=c("AFR","EUR","SAS","EAS","AMR"),col=c("blue","grey","red","yellow","green")),col=col_fun,scale="row",margins=c(3,7),cexRow=0.8,cexCol=1,file="heatmap.pdf")
  }
    dev.off()
}
...
[Minor allele frequency distribution of asthma-associated SNPs for ASW, CEU, and YRI.](change-this!!!!!!!!!!!image_url)
...
myPop <- allPops(data) # make a list of all 26 populations included in the study
...
makeHeatmap(sigRes,continentColors)
knitr::include_graphics("heatmap.jpg")
```
...
[Heatmap of significantly enriched or depleted alleles from the original publication.](change-to-url!!!!)
