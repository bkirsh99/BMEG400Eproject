...
By utilizing pre-compiled, but not pre-processed data, we overcame computationally-intensive and resource-demanding challenges associated with retrieving data from the 1000 Genomes Project. At the same time, we ensured that this re-analysis was conducted on the raw data set used by the original publication prior to major filtering or manipulation steps.
...
We ran the pipeline for the three pre-selected populations, CEU, ASW, and YRI, as illustrated below.
...
The results from hypergeometric testing were also utilized to generate a heat map and visualize allele enrichment and depletion patterns among the three populations (Figure 3). If an effect allele was enriched, the cell was colored red based on the negative value of log10P, whereas if an effect allele was depleted, it was colored green based on the actual value of log10P. A hierarchical clustering of the three populations shows that the YRI and ASW populations share similar allele enrichment and depletion patterns. Furthermore, many cases of simultaneous significant enrichment in one population and depletion in another are observed in the CEU and YRI populations. In other words, they exhibit multiple SNPs with opposite directions in allele frequency changes, where effect alleles that are enriched in one population appear depleted in the other. In these scenarios, the ASW population has a tendency to bias the enrichment or depletion status of effect alleles in the CEU population.

As hypothesized by the original publication, where ASW and YRI are tightly clustered together and distant from the CEU cluster, we believe that genetic factors related to the ancestry of continental populations influences obesity-related SNP enrichment and depletion patters. The code block below indicates how the heat map was generated.

```{r, eval=TRUE,tidy = TRUE, cache=FALSE, fig.cap="A heat map of significantly enriched and depleted effect alleles across the 3 populations", out.width = '100%', message=F, warning=F}
...
To second investigation consisted of analyzing the differences in effect allele frequency by plotting their distributions across the three populations (Figure 4). Only a small subset of SNPs was considered at this stage, as determined by their significance in both the enrichment analysis and GWAS studies. The code block below indicates how the distribution plot was generated.
...
```{r, eval=TRUE, tidy = TRUE, cache=FALSE, fig.cap="A frequency distribution plot of effect alleles for the most significant SNPs", message=F, warning=F}
distPlot <- plotDist(sigRes,freqInfo)
distPlot

```

The minor allele frequency (MAF) distribution of asthma-related GWAS SNP's across ASW, CEU, and YRI populations is shown below for comparison.

[Minor allele frequency distribution of asthma-associated SNPs for ASW, CEU, and YRI.](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC4307746/bin/40246_2014_23_Fig5_HTML.jpg)
```{r, eval=TRUE,tidy = TRUE, cache=FALSE, fig.cap="The minor allele frequency (MAF) distribution of asthma-related GWAS SNP's across ASW, CEU, and YRI populations", out.width = '100%', message=F, warning=F}


z <- tempfile()

download.file("www.ncbi.nlm.nih.gov/pmc/articles/PMC4307746/bin/40246_2014_23_Fig5_HTML.jpg",z, mode = 'wb')
...
Lastly, to confirm that the hypergeometric tests and clustering methods applied to the 3 populations were in line with those from the original publication, we also generated a heat map of the enrichment/depletion p-values (log10 based) of effect alleles in all 26 populations. The use of a pipeline for most aspects of the project greatly facilitated this step, as the only parameter to be modified was the populations under investigation.

This final stage of the re-analysis was crucial to validate the conclusions made using the replicated methods for a smaller subset of sampled populations, as it would reveal whether any inconsistencies were due to a faulty approach or simply an inability to reproduce the analysis with a different data set.
...
```{r, eval=TRUE,tidy = TRUE, cache=FALSE, fig.cap="A heat map of significantly enriched and depleted effect alleles across all 26 populations", out.width = '100%', message=F, warning=F}
...
The original heat map showing significant enrichment and depletion of the effect alleles of 225 obesity risk SNPs across the 26 populations is shown below for comparison.

[Heatmap of significantly enriched or depleted alleles from the original publication.]("heatmap_original.jpg")
```{r, eval=TRUE,tidy = TRUE, cache=FALSE, fig.cap="The original heat map showing significant enrichment and depletion of the effect alleles of 225 obesity risk SNPs across the 26 populations", out.width = '100%', message=F, warning=F}
...
In general, the results of this re-analysis are consistent with the literature. The additional study that investigated asthma-related variants in precisely the same three populations, ASW, CEU, and YRI, came to strikingly similar conclusions.
...
Thus, we conclude that ancestral genetics do play a role in the disease susceptibility, to some extent. It is important to caveat that obesity and obesity-related traits are the product of combined effects of multiple different genetic variants and their interaction with surrounding environmental variables, so environmental factors such as diet, climate, local pathogens and lifestyle are other large contributors .
